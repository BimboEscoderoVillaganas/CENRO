    ...
    <li class="sidebar-item">
        ...
        <div id="content">
            ...
            <div class="menu-header">
                ...
                <span class="menu-text">Archive Queue</span>
...
$query = "SELECT d.document_id, f.file_name, d.document_number, d.approving_authority, 
          ...
          WHERE (LOWER(d.deleted) <> 'yes' OR d.deleted IS NULL)
          AND (
              -- For numeric year values (like '2' in sample data)
              (d.retention_schedule REGEXP '^[0-9]+$' AND d.retention_schedule < $currentYear)
              OR
              -- For date values (like '2011-06-30' in sample data)
              (STR_TO_DATE(d.retention_schedule, '%Y-%m-%d') IS NOT NULL 
               AND STR_TO_DATE(d.retention_schedule, '%Y-%m-%d') < '$currentDate')
              OR
              -- For invalid or non-date formats (treated as expired)
              (d.retention_schedule NOT REGEXP '^[0-9]+$' 
               AND STR_TO_DATE(d.retention_schedule, '%Y-%m-%d') IS NULL
...
if ($result && mysqli_num_rows($result) > 0):
    while ($row = mysqli_fetch_assoc($result)):
        // Create cabinet display text
        $cabinetDisplay = ($row['cabinet_code'] && $row['cabinet_location']) 
...
?>
        ...
        ?>
            <tr>
                <td colspan="12" class="text-center">No expired documents found.</td>
            </tr>
        <?php endif; ?>
        </tbody>
    </table>
